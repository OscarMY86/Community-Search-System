...
def pre_com(data_set='com_dblp',subgraph_list=[400], train_ratio=0.02,seed_cnt=20,cmty_size=30):
    ...
    print(f"Load from pre_com {data_set} edges")
    ...
    print(f"Load from pre_com {data_set} cmty")
...
def untar_snap_data(name):
    ...
    with open(root / 'dblp-graph.txt', 'r') as fh:
    ...
    print("Now doing fmapping\n")
    with open('grp/m.txt', 'w') as file:
